...
const generateGRNSheetFromGRN = (grn) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      const supplierY = doc.y;
      doc.font('Helvetica-Bold').fontSize(10).text('Supplier Name:', 50, supplierY);
      doc.font('Helvetica').text(grn.supplier_name || 'N/A', 160, supplierY);
      doc.moveDown(1);
      ...
      const itemRowY = itemTableTop + 25;
      doc.font('Helvetica').fontSize(9);
      doc.text(grn.item_name || 'N/A', 55, itemRowY, { width: 115 });
      doc.text(grn.item_description || 'N/A', 170, itemRowY, { width: 120 });

      doc.moveTo(50, itemRowY + 20).lineTo(545, itemRowY + 20).stroke();

      // Vertical lines for item table
      [50, 165, 545].forEach(x => {
        doc.moveTo(x, itemTableTop).lineTo(x, itemRowY + 20).stroke();
      });
      ...
      const receivedQty = grn.received_quantity != null ? parseFloat(grn.received_quantity) : 0;
      const checkedQty = grn.checked_quantity != null ? parseFloat(grn.checked_quantity) : 0;
      const acceptedQty = grn.accepted_quantity != null ? parseFloat(grn.accepted_quantity) : 0;
      const rejectedQty = grn.rejected_quantity != null ? parseFloat(grn.rejected_quantity) : 0;
      ...
      // Invoice details
      doc.font('Helvetica-Bold').text('Invoice Number:', 50, doc.y);
      doc.font('Helvetica').text(grn.invoice_number || 'N/A', 160, doc.y);
      doc.moveDown(0.3);
      doc.font('Helvetica-Bold').text('Invoice Date:', 50, doc.y);
      doc.font('Helvetica').text(grn.invoice_date || 'N/A', 160, doc.y);
      doc.moveDown(0.5);

      // Remarks
      doc.font('Helvetica-Bold').text('GRN Remarks:', 50, doc.y);
      doc.font('Helvetica').text(grn.remarks || 'N/A', 160, doc.y);
