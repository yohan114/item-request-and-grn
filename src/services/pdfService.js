...
const generateMRNSheetFromMRN = (mrn) => {
  return new Promise((resolve, reject) => {
    try {
      const doc = new PDFDocument({ margin: 50, size: 'A4' });
      const buffers = [];
...
const generateGRNSheetFromGRN = (grn) => {
  return new Promise((resolve, reject) => {
    try {
      const doc = new PDFDocument({ margin: 50, size: 'A4' });
      const buffers = [];
      ...
      doc.font('Helvetica-Bold').fontSize(10);
      doc.text('GRN Number: ', 50, infoY);
      doc.font('Helvetica').text(grn.grn_number || 'N/A', 140, infoY);

      doc.font('Helvetica-Bold').text('Date: ', 350, infoY);
      doc.font('Helvetica').text(
        grn.created_at ? new Date(grn.created_at).toISOString().slice(0, 10) : new Date().toISOString().slice(0, 10),
        390,
        infoY
      );
      ...
      // Supplier Name
      doc.font('Helvetica-Bold').fontSize(10).text('Supplier Name:', 50, doc.y);
      doc.font('Helvetica').text(grn.supplier_name || 'N/A', 160, doc.y);
      doc.moveDown(0.5);

      // Project Name
      doc.font('Helvetica-Bold').fontSize(10).text('Project Name:', 50, doc.y);
      doc.font('Helvetica').text(grn.project_name || 'N/A', 160, doc.y);
      doc.moveDown(1);
      ...
      if (grn.items) {
        if (Array.isArray(grn.items)) {
          items = grn.items;
        } else if (typeof grn.items === 'string') {
          try { items = JSON.parse(grn.items); } catch (e) { items = []; }
        }
      }

      // Item table header
      const itemTableTop = doc.y;
      const colWidths = [80, 235, 100, 80];
      const colStarts = [50, 130, 365, 465];

      doc.font('Helvetica-Bold').fontSize(9);
      doc.text('Item No', colStarts[0] + 5, itemTableTop + 5, { width: colWidths[0] - 10 });
      doc.text('Description', colStarts[1] + 5, itemTableTop + 5, { width: colWidths[1] - 10 });
      doc.text('Quantity', colStarts[2] + 5, itemTableTop + 5, { width: colWidths[2] - 10 });
      doc.text('Price', colStarts[3] + 5, itemTableTop + 5, { width: colWidths[3] - 10 });
      ...
      if (items.length === 0) {
        ...
        items.forEach((item) => {
          ...
          doc.text(String(item.description || ''), colStarts[1] + 5, rowY, { width: colWidths[1] - 10 });
          doc.text(String(item.qty || 0), colStarts[2] + 5, rowY, { width: colWidths[2] - 10 });
          doc.text(String(item.price || 0), colStarts[3] + 5, rowY, { width: colWidths[3] - 10 });
          rowY += 20;
          doc.moveTo(50, rowY).lineTo(545, rowY).stroke();
        });
      }
      ...
      // Vertical lines for item table
      [50, colStarts[1], colStarts[2], colStarts[3], 545].forEach(x => {
        doc.moveTo(x, itemTableTop).lineTo(x, rowY).stroke();
      });
      ...
      const reqStartY = doc.y;
      doc.font('Helvetica-Bold').fontSize(10).text('Name:', 50, reqStartY);
      doc.font('Helvetica').text(grn.request_person_name || '________________________', 130, reqStartY);
      doc.moveDown(0.5);
      doc.font('Helvetica-Bold').text('Designation:', 50, doc.y);
      doc.font('Helvetica').text(grn.request_person_designation || '________________________', 130, doc.y);
      doc.moveDown(0.5);
      doc.font('Helvetica-Bold').text('Signature:', 50, doc.y);
      doc.font('Helvetica').text('________________________', 130, doc.y);
      doc.moveDown(0.5);
      ...
      const appStartY = doc.y;
      doc.font('Helvetica-Bold').fontSize(10).text('Name:', 50, appStartY);
      doc.font('Helvetica').text(grn.approval_person_name || '________________________', 130, appStartY);
      doc.moveDown(0.5);
      doc.font('Helvetica-Bold').text('Designation:', 50, doc.y);
      doc.font('Helvetica').text(grn.approval_person_designation || '________________________', 130, doc.y);
      doc.moveDown(0.5);
      doc.font('Helvetica-Bold').text('Signature:', 50, doc.y);
      doc.font('Helvetica').text('________________________', 130, doc.y);
      doc.moveDown(0.5);
...
module.exports = { generateMRNSheet, generateGRNSheet, generateMRNSheetFromMRN, generateGRNSheetFromGRN };
